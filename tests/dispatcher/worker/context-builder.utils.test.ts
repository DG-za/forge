...
import { buildWorkerContext } from '@/dispatcher/worker/context-builder.utils';
...
describe('buildWorkerContext', () => {
  it('should include issue title and body', async () => {
    ...
    const context = await buildWorkerContext('/worktree.utils', task, exec);
  ...
  it('should read files from the worktree path', async () => {
    ...
    await buildWorkerContext('/worktree.utils', task, exec);
